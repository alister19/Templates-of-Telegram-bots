...
async def process_cancel_command(message: Message):
    if users[message.from_user.id]['in_game']:
        ...
        await message.answer(
            'А мы так с вами не играем '
            'Может, сыграем разок?'
        )

# Этот хэндлер будет срабатывать на согласие пользователя 
# сыграть в игру
...
async def process_positive_answer(message: Message):
    if not users[message.from_user.id]['in_game']:
        ...
        await message.answer(
            'Пока мы играем в игру ямогу '
            'реагировать только на числа от 1 до 100 '
            'и команды /cancel и /stat'
        )

# Этот хэндлер будет срабатывать на отказ пользователя
# сыграть в игру
...
# Этот хэндлер будет срабатывать на отправку
# пользователем чисел от 1 до 100
...
async def process_other_answer(message: Message):
    if users[message.from_user.id]['in_game']:
        ...
        await message.answer(
            ...
            'просто сыграем в игру? Достал уже '
            'кожаный мудак)))'
        )
